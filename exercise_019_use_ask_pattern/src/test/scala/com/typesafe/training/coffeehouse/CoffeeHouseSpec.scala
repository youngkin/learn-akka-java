...
class CoffeeHouseSpec extends BaseAkkaSpec {
  ...
  "Sending CreateGuest to CoffeeHouse" should {
    "result in creating a Guest" in {
      ...
      coffeeHouse ! new CoffeeHouse.CreateGuest(new Coffee.Akkaccino, Int.MaxValue)
  ...
  "On failure of Guest CoffeeHouse" should {
    "stop it" in {
      ...
      coffeeHouse ! new CoffeeHouse.CreateGuest(new Coffee.Akkaccino, 0)
      ...
      actor("resend-prepare-coffee")(new CoffeeHouse(Int.MaxValue) {
        ...
        override def createWaiter() =
          actor(context, "waiter")(new Act {
            become { case _ => throw new Waiter.FrustratedException(new Coffee.Akkaccino, system.deadLetters) }
          })
      })
      val waiter = TestProbe().expectActor("/user/resend-prepare-coffee/waiter")
      waiter ! "blow-up"
      barista.expectMsg(new Barista.PrepareCoffee(new Coffee.Akkaccino, system.deadLetters))
    }
  }
  ...
  "Sending GetStatus to CoffeeHouse" should {
    "result in a Status response" in {
      ...
      val coffeeHouse = actor("get-status")(new CoffeeHouse(Int.MaxValue))
      coffeeHouse ! CoffeeHouse.GetStatus.Instance
